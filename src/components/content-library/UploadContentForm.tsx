...
/** Must match backend MEDIA_UPLOAD_MAX_BYTES (8 GB). */
const MAX_VIDEO_BYTES = Math.floor(8 * 1024 * 1024 * 1024);
...
export function UploadContentForm({
  ...
  function handleFileChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (selected && selected.size > MAX_VIDEO_BYTES) {
      setFile(null);
      onReadyChange?.(false);
      setFileError('Video must be 8 GB or smaller');
      event.target.value = '';
      return;
    }
    ...
    >
      ...
      <div>
        ...
        >
          ...
          <span className="text-caption text-content-muted">MP4, MOV, or WebM · max 8 GB</span>
        </label>
        {fileError && (
